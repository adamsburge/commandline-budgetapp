...
At the start of the app, the user is presented with a 4 options: 
1. Log in
2. Create an account
3. Delete my account
4. Quit App
...
If the user opts to adjust their categories, the app will guide them through the process of either adding a new budget category or deleting one (and consequently rebudgeting any money which the deleted category had).

### Adjust Budget to Match Bank Balance
![Update Balance](assets/images/update_balance.png)

If the user has forgotten to budget for a while and their current bank account doesn't match the budgeted amount, they can select the option to update their bank balance. This will then calculate whether they need to add or deduct money from the budget and prompt the user to do accordingly.
...
3. Allowed two other people (my wife, Megan, and my friend, Michael) to attempt to break the code by inserting incorrect entries throughout. After several modifications and debugging sessions, the code was able to withstand incorrect inputs throughout and redirect the user to input a correct input.
4. Tested the app in my local terminal and in the Code Institute Heroku terminal

## Bugs

### Solved Bugs
1. When I began the project, I had not planned to make the budget allow different users to create their own budgets. But after conversations about the functionality of the app with my mentor, I decided to create this functionality to allow the users to manage their own budget.
